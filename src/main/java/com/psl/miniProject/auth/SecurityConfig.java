package com.psl.miniProject.auth;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.psl.miniProject.auth.models.SecurityProperties;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        System.out.println(restSecProps.getAllowedPublicApis().toArray(String[]::new));
        http.cors().configurationSource(corsConfigurationSource()).and().csrf().disable().formLogin().disable()
        .httpBasic().disable().exceptionHandling().authenticationEntryPoint(restAuthenticationEntryPoint())
        .and().authorizeRequests()
        // .antMatchers().permitAll()
        // .antMatchers(HttpMethod.GET,"/products/*").permitAll().and()
        .antMatchers(restSecProps.getAllowedPublicApis().toArray(String[]::new)).permitAll()
        .antMatchers(HttpMethod.OPTIONS, "/**").permitAll().anyRequest().authenticated().and() .addFilterBefore(
